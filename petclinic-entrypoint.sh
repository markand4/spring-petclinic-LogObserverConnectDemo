...
if [ -f "/app/downloaded-agents/appdynamics-agent/javaagent.jar" ]; then
    ...
    JAVA_OPTS="$JAVA_OPTS -Dappdynamics.controller.ssl.enabled=${APPD_SSL_ENABLED:-${APPD_SSL:-true}}"
    JAVA_OPTS="$JAVA_OPTS -Dappdynamics.agent.accountName=${APPD_ACCOUNT_NAME:-your-account}"
    JAVA_OPTS="$JAVA_OPTS -Dappdynamics.agent.accountAccessKey=${APPD_ACCESS_KEY:-${APPD_KEY:-your-access-key}}"
